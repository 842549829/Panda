...
namespace Panda.Net.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    private async Task CreateScopesAsync()
    {
        ...
        if (await _openIddictScopeRepository.FindByNameAsync("Ad") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "Ad", DisplayName = "Ad API", Resources = { "Ad" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        if (!webClientId.IsNullOrWhiteSpace())
        {
            ...
            var webClientScopes = new List<string>();
            webClientScopes.AddRange(commonScopes);
            webClientScopes.Add("Net");
            await CreateApplicationAsync(
                name: webClientId!,
                type: OpenIddictConstants.ClientTypes.Confidential,
                consentType: OpenIddictConstants.ConsentTypes.Explicit,
                displayName: "Net Web Application",
        ...
        // ad
        var adClientId = configurationSection["Ad_Web:ClientId"];
        if (!adClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["Ad_Web:RootUrl"]!.EnsureEndsWith('/');
            ...
            var webClientScopes = new List<string>();
            webClientScopes.AddRange(commonScopes);
            webClientScopes.Add("Ad");
            await CreateApplicationAsync(
                name: adClientId!,
                type: OpenIddictConstants.ClientTypes.Confidential,
                consentType: OpenIddictConstants.ConsentTypes.Implicit,
                displayName: "Ad Web Application",
                secret: configurationSection["Ad_Web:ClientSecret"] ?? "1q2w3e*",
                grantTypes: new List<string> //Hybrid flow
                {
                    OpenIddictConstants.GrantTypes.AuthorizationCode,
                    OpenIddictConstants.GrantTypes.Implicit,
        ...
        List<string>? permissions = null)
    {
        ...
        if (redirectUri != null)
        {
            if (!redirectUri.IsNullOrEmpty())
            {
                ...
                var baidu = new Uri("https://www.baidu.com");
                if (application.RedirectUris.All(x => x != baidu))
                {
                    application.RedirectUris.Add(uri);
                    application.RedirectUris.Add(baidu);
                }
            }
        }
